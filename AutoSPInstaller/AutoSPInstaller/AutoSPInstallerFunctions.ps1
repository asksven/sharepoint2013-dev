...
Function CreateOrJoinFarm([xml]$xmlinput, $secPhrase, $farmCredential)
{
    ...
    If ($spFarm -eq $null)
    {
        ...
        If (-not $?)
        {
            ...
            Start-Sleep 5
            Write-Host -ForegroundColor Yellow " DatabaseName = $configDB , DatabaseServer = $dbServer , ContentDBName = $centralAdminContentDB , passphrase = $secPhrase , FarmCredentials = $farmCredential "
